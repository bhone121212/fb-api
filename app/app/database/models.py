...
class Post(db.Model):
    ...
    date = Column('date', db.DateTime)
    last_time_updated = Column('last_time_updated', db.DateTime)
...
class Task(db.Model):
    __tablename__ = 'tasks'
    id = Column('id', db.Integer, primary_key=True)
    interval = Column('interval', db.Integer)
    retro = Column('retro', db.DateTime)
    until = Column('until', db.DateTime)
    sent_time = Column('sent_time', db.DateTime)
    received_time = Column('received_time', db.DateTime)
    finish_time = Column('finish_time', db.DateTime)
    status = Column('status', ENUM(TaskStatus))
    enabled = Column('enabled', db.Boolean)
...
class Comment(db.Model):
    ...
    parent_comment_id = Column('parent_comment_id', db.Integer)
    content_id = Column(Integer, ForeignKey('content.id'))
    likes_count = Column('likes_count', db.Integer)
    user_id = Column(Integer, ForeignKey('users.id'))
    post_id = Column(Integer, ForeignKey('posts.id'))
    content = relationship("Content")
    user = relationship("User")
...
class WorkerCredential(db.Model):
    ...
    id = Column('id', db.Integer, primary_key=True)
    account_id = Column('account_id', db.Integer)
    proxy_id = Column('proxy_id', db.Integer)
    user_agent_id = Column('user_agent_id', db.Integer)
    inProgress = Column('inProgress', db.Boolean)
    inProgressTimeStamp = Column('in_progress_timestamp', db.DateTime)
    locked = Column('locked', db.Boolean)
